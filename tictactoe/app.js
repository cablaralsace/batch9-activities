...
function handleClick(e) {
  ...
  //empty array para every click marerefresh and di lang yung latest data makukuha
  gameTurn = [];
  ...
  if (winner === false) {
    ...
    if (gameTurn.index >= 0 && gameTurn.index <= 2) {
      //row: 0, col: 0,1,2
      row = 0;
      col = gameTurn.index;
    } else if (gameTurn.index >= 3 && gameTurn.index <= 5) {
      //row: 0, col: 3,4,5 - 3 = 0,1,2
      row = 1;
      col = gameTurn.index - 3;
    } else {
      //row: 0, col: 6,7,8 - 6 = 0,1,2
      row = 2;
    ...
    if (checkWin(currentClass)) {
      ...
      //magpapalit ng X or O and may show hover
      swapTurns();
      setBoardHoverClass();
    }
  }
...
function endGame(draw) {
  if (draw) {
    //display text pag Draw
    winningMessageElement.classList.add("color");
    winningMessageTextElement.innerText = "Draw!";
  } else {
    //display text pag X or O panalo
    winningMessageElement.classList.add("color");
    winningMessageTextElement.innerText = `Congratulations! ${
...
function isDraw() {
  //checks kung may laman lahat ng cells
...
function placeMark(cell, currentClass) {
  if (winner === false) {
    //if wala pang winner, place lang nang place nang mark
...
function swapTurns() {
  ...
  winningMessageTextElement.innerText = `${
    oTurn ? "Player O's" : "Player X's"
  } Turn`; //shows Player Turn as well
  winningMessageElement.classList.add("show");
}

function setBoardHoverClass() {
  //hover X or O as long as di pa tapos yung laro or wala pang winner/draw
  board.classList.remove(xClass);
  board.classList.remove(oClass);
...
function checkWin(currentClass) {
  return WINNING_COMBINATIONS.some((combination) => {
    //checks winning combinations array
    return combination.every((index) => {
      //kung may laman na X or O lahat ng winning combination indices
